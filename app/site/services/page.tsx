      ...
      <section className="hero-panel min-h-[72vh] pt-28 pb-16 md:min-h-[85vh] md:pt-36 md:pb-24">
        <div className="container-main">
          ...
          <p className="mt-8 max-w-prose text-[16px] leading-[1.7] text-light-slate">
            {data.heroDescription || "A 4-6 week structured engagement covering one business domain. Stands alone as a governance diagnostic, or becomes the foundation for broader architecture work."}
          </p>
          ...
          <div className="grid gap-16 lg:grid-cols-2 lg:gap-14">
            ...
            <div>
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                {(data.tags || ["4-6 weeks", "One business domain", "Fixed scope", "Five deliverables", "Platform-agnostic"]).map((tag: string) => (
                  <span key={tag} className="rounded-btn border border-light-slate bg-mid-blue/5 px-3 py-1 text-xs font-medium text-slate">{tag}</span>
                ))}
              </div>
              ...
              <ol className="mt-6 border-light-slate">
                {processSteps.map((step: any, index: number) => (
                  <li key={step.number || index} className={`border-light-slate py-5 ${index > 0 ? "border-t" : ""}`}>
